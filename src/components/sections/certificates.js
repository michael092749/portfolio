...
const Certificates = ({ data }) => {
  ...
  const GRID_LIMIT = 9;
  const certificates = data.filter(({ node }) => node);
  const firstSix = certificates.slice(0, GRID_LIMIT);
  const projectsToShow = showMore ? certificates : firstSix;

  return (
    <StyledContainer id='certificates'>
            <Heading>Certificates</Heading>

      {/* <StyledTitle ref={revealTitle}>Certificates</StyledTitle> */}
      <StyledGrid>
        <TransitionGroup className="certificates">
          {projectsToShow &&
